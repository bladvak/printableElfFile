...
std::string PrintableEhdr32::intToStrHex(long long val)
{
...
PrintableEhdr32::PrintableEhdr32(void *mem_ptr) : ehdr32_ptr(static_cast<Elf32_Ehdr *>(mem_ptr))
{
    if(ehdr32_ptr == nullptr)
        throw std::runtime_error("pointer to Ehdr struct cannot be null.");
}

 std::string PrintableEhdr32::magic()
 {
    std::ostringstream ss;
    for(int i=0; i<16; ++i)
        ss<<"0x"<<std::hex<<(int)ehdr32_ptr->e_ident[i];
    return ss.str();
 }

 std::string PrintableEhdr32::ident_class()
 {
    switch(ehdr32_ptr->e_ident[EI_CLASS]){
 ...
 std::string PrintableEhdr32::ident_data()
 {
    switch(ehdr32_ptr->e_ident[EI_DATA]){
 ...
 std::string PrintableEhdr32::ident_version()
 {
    switch(ehdr32_ptr->e_ident[EI_VERSION]){
 ...
 pr PrintableEhdr32::type()
 {
    std::string result;

    switch(ehdr32_ptr->e_type){
    ...
    return {intToStrHex(ehdr32_ptr->e_type), result};
 }

 pr PrintableEhdr32::machine()
 {
    return {intToStrHex(ehdr32_ptr->e_machine),e_type[ehdr32_ptr->e_machine]};
 }

 pr PrintableEhdr32::version()
 {
    std::string result;
    switch(ehdr32_ptr->e_version){
      case EV_NONE: result = "NONE"; break;
      case EV_CURRENT: result = "CURRENT"; break;
      case EV_NUM: result = "NUM"; break;
      default: result = "---"; break;
    }
    return {intToStrHex(ehdr32_ptr->e_version), result};
 }

 std::string PrintableEhdr32::entry()
 {
    return std::to_string(ehdr32_ptr->e_entry);
 }

 std::string PrintableEhdr32::phoff()
 {
    return std::to_string(ehdr32_ptr->e_phoff);
 }

 std::string PrintableEhdr32::shoff()
 {
    return std::to_string(ehdr32_ptr->e_shoff);
 }

 std::string PrintableEhdr32::flags()
 {
    return std::to_string(ehdr32_ptr->e_flags);
 }

 std::string PrintableEhdr32::ehsize()
 {
    return std::to_string(ehdr32_ptr->e_ehsize);
 }

 std::string PrintableEhdr32::phentsize()
 {
    return std::to_string(ehdr32_ptr->e_phentsize);
 }

 std::string PrintableEhdr32::phnum()
 {
    return std::to_string(ehdr32_ptr->e_phnum);
 }

 std::string PrintableEhdr32::shentsize()
 {
    return std::to_string(ehdr32_ptr->e_shentsize);
 }

 std::string PrintableEhdr32::shnum()
 {
    return std::to_string(ehdr32_ptr->e_shnum);
 }

 std::string PrintableEhdr32::shstrndx()
 {
    return std::to_string(ehdr32_ptr->e_shstrndx);
 }

std::unordered_map<int, std::string> PrintableEhdr32::e_type{
    {EM_NONE,"No machine "},      
